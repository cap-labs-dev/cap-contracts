...
library ViewLib {
    ...
    function claimable(IMarket.Storage storage $, address tranche) external view returns (uint256 reward) {
        ...
        if ($.lastRewardUpdate != block.timestamp) {
            ...
            uint256 supplyIndex_ = supplyIndex($);
            uint256 underwriterIndex_ = underwriterIndex($);
            uint256 scaledDebt = $.scaledDebt;
            uint256 premiumInterest = scaledDebt.rayMul(supplyIndex_.rayMul(underwriterIndex_ - $.lastUnderwriterIndex));
