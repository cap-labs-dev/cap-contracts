...
interface IOracle {
    ...
    struct PriceOracleStorage {
        mapping(address => OracleData) oracleData;
        mapping(address => OracleData) backupOracleData;
        mapping(address => uint256) staleness;
    }

    /// @notice Oracle data for an asset
    ...
    /// @dev Set oracle data
    event SetPriceOracleData(address asset, OracleData data);

    /// @dev Set backup oracle data
    event SetPriceBackupOracleData(address asset, OracleData data);
    ...
    function setPriceOracleData(address _asset, OracleData calldata _oracleData) external;
    ...
    function setPriceBackupOracleData(address _asset, OracleData calldata _oracleData) external;
    ...
    function priceOracleData(address _asset) external view returns (OracleData memory data);
    ...
    function priceBackupOracleData(address _asset) external view returns (OracleData memory data);
