...
/// @dev Symbiotic adapter deployment + access-control wiring.
contract DeploySymbioticNetworkAdapter is ProxyUtils {
    /// @dev Deploys Symbiotic proxy implementations (logic contracts).
    ...
    /// @dev Deploys + initializes Symbiotic adapter proxies and the CAP Symbiotic vault factory.
    ...
    /// @dev Grants minimal permissions so the Symbiotic adapter can act through CAP's `AccessControl`.
    ...
    /// @dev Registers a deployed Symbiotic vault into the middleware for an `agent`.
