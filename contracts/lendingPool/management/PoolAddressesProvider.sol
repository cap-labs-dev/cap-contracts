 ...
 */
contract PoolAddressesProvider is OwnableUpgradeable, IPoolAddressesProvider {
  ...
  bytes32 private constant AVS = 'AVS';
  ...
  function getAvs() external view override returns (address) {
    return getAddress(AVS);
  }

  /// @inheritdoc IPoolAddressesProvider
  function setAvs(address newAvs) external override onlyOwner {
    address oldAvs = _addresses[AVS];
    _addresses[AVS] = newAvs;
    emit AVSUpdated(oldAvs, newAvs);
  }
