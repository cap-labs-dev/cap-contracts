...
3. **The PoC** — run it (`FOUNDRY_TEST=audit/tests/scratch/<WS> forge test --match-path
   'audit/tests/scratch/<WS>/<file>' -vv`). Read it line by line. Does it demonstrate the claimed
...
Write `audit/findings/verify/<ID>.md` with EXACTLY this structure:
...
`audit/tests/scratch/verify/<ID>/`. Run it with `FOUNDRY_TEST=audit/tests/scratch/verify/<ID>`.
