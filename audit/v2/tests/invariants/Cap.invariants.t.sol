...
contract CapInvariants is StdInvariant, CapDeployer, ICapWorld {
    ...
    function trancheRequestIdsOf(address t) external view returns (uint256) {
        return _trancheRequests[t];
    }
    ...
    /// I17 (H-2): queued claims never exceed live unlocked supply on tranches
    function invariant_I17_trancheClaimableWithinUnlocked() public view {
        _i17(senior);
        _i17(junior);
        _i17(fSenior);
        _i17(fJunior);
    ...
    function _i17(Tranche t) internal view {
        uint256 n = handler.trancheRequestIds(address(t));
        uint256 sum;
        for (uint256 id; id < n; ++id) {
            for (uint256 u; u < uws.length; ++u) {
                try t.claimableRedeemRequest(id, uws[u]) returns (uint256 c) {
                    sum += c;
                }
                    catch {
                    return;
        ...
        uint256 unlocked;
        try t.unlockedSupply() returns (uint256 x) {
            unlocked = x;
        }
            catch {
            return;
        }
        assertLe(sum, unlocked, "I17: claimable exceeds unlockedSupply (H-2)");
    }

    /// I18: staked supply == sum of opted-in balances (tranches; underwriters opt in via _fundTranche)
    ...
    function _i18(Tranche t) internal view {
        uint256 s;
        for (uint256 u; u < uws.length; ++u) {
            if (t.optedIn(uws[u])) s += t.balanceOf(uws[u]);
        }
        assertEq(t.stakedSupply(), s, "I18");
    }
    ...
    function _i19(Tranche t) internal view {
        // pot held >= what is still owed: remaining (unvested) + every holder's claimable
        uint256 owed = t.remaining();
        for (uint256 u; u < uws.length; ++u) {
            owed += t.claimable(uws[u]);
        }
        assertLe(
            owed, IERC20(address(stablecoin)).balanceOf(address(t)) + 1e3, "I19: entitlements exceed pot beyond dust"
        );
    }
