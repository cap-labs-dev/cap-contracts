...
/// Round-2 port of round-1 V_LoanIdReuse (M-5). Assertions identical; API changes only:
/// `capConfig.stablecoinYield` is gone -- the liquidity premium is now `fundCreditBacked` minted to the
/// stablecoin itself, so `stablecoin.balanceOf(address(stablecoin))` is the equivalent measure; and the
/// manual tranche deposit adds `optIn()` to mirror the new `_fundTranche` harness.
...
contract R1_M5b_LoanIdReuse is CapDeployer {
    FixedMarket market;
    address tranche0;
    ...
    function test_keeperCanSeedInsteadOfOwner_noHealthCheck() public {
        ...
        // the real oracle (not round 1's MockOracle) has a staleness window; re-post the same price
    ...
    function test_quantify_10M_splitByRecipient() public {
        ...
        uint256 stcBefore = stablecoin.balanceOf(address(stablecoin));
        ...
        uint256 paidL = stablecoin.balanceOf(address(stablecoin)) - stcBefore;
