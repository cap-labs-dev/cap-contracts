...
// L-17 (round 2): the EMA weight is now `1 - retention^elapsed` (WadRayMath.rayPow), so the
// average after a window should be the same however many accruals split that window.
...
/// @dev Ported round-1 E4::test_EMA_weightDependsOnAccrualFrequency against the real deployment
contract L17_EmaRegime is CapDeployer {
    ...
    function test_EMA_weightDependsOnAccrualFrequency() public {
        ...
        // settle: a full window of retention decay is not enough for exact convergence under the
        // new exponential kernel (residual ~1/e per window), so settle across many windows
        for (uint256 i; i < 40; ++i) {
            skip(period);
            irm.updateLiquidityRate();
        }
        (, uint256 s0) = irm.averageSupplies();
        assertApproxEqRel(s0, 1_500_000e18, 1e9, "settled at 1M deposited + 500k credit-backed");

        uint256 snap = vm.snapshotState();
        _depositStable(makeAddr("d"), 1_000_000e18); // observed supply -> 2.5M
        skip(period);
        ...
        // identical in meaning to round 1 (assertEq), allowing only ray rounding
        assertApproxEqRel(
            busySupply,
            quietSupply,
            1e9,
...
/// @dev Property fuzz on the real InterestRateModel behind a thin stablecoin harness
contract L17_EmaPathIndependence is Test {
    ...
    /// @param P total elapsed window, [1, 365 days]
    /// @param k number of evenly spaced accruals summing to P, [1, 64]
    /// @param obs the observed total supply the EMA tracks
    function testFuzz_emaPathIndependent(uint256 P, uint256 k, uint256 obs) public {
        P = bound(P, 1, 365 days);
        k = bound(k, 1, 64);
        obs = bound(obs, 1e18, 1e15 * 1e18); // 1 token .. 1e15 tokens
        uint256 credit = obs / 3;

        // seed a non-zero starting average so the test exercises both `_carry` branches
        ...
        // each accrual rounds at most 1 wei (rayMul half-up), so allow k wei absolute or 1e-9 relative
