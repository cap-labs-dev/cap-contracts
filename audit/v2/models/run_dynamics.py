...
from capmath import StablecoinState, WAD, DAY, HOUR, RAY, rayDiv  # noqa: E402
...
def part_c_jit():
    """3dad5ef: premium vests 1 - (1 - 1/12h)^t to opted-in holders; optIn() sets debt at the current
    perShare so nothing already vested is captured. A fixed 30-day loan mints its WHOLE liquidity
    premium P at borrow (front-loaded), so a JIT depositor who opts in right after the borrow with
    J = H (matching the honest opted-in stake, 50% of staked) takes 0.5 * w(h) * P for holding h."""
    from capmath import vesting_weight
    from ema_manipulation import premium_for
    P = premium_for(10_000_000 * WAD, 30 * DAY, rayDiv(90_000_000 * WAD + 10_000_000 * WAD, 100_000_000 * WAD + 10_000_000 * WAD), 0)
    print("\n== C. JIT depositor vs the 12 h exponential opt-in vest (3dad5ef) ==")
    print("Pot P = liquidity premium of a $10M 30-day fixed borrow at u=90%%: $%.0f, minted at borrow." % (P / WAD))
    print("JIT: opt-in with J = honest stake H right after the borrow, hold h, claim, opt-out/redeem at par (no fee).")
    print("Round-1 M-3 (settable vestingPeriod, 6 h default): 50% of the premium for 6 h. Now:")
    print("  hold h | w(h)    | JIT take (50% of vested) | $ at P | per-6h-equivalent | vs time-pro-rata over T=1d / 7d / 30d")
    r_alt = 0.05
    for h in (1, 3, 6, 12, 24, 48, 96):
        w = vesting_weight(h * HOUR) / RAY
        take = 0.5 * w
        cols = []
        for T in (1, 7, 30):
            fair = 0.5 * min(h / (24 * T), 1.0)  # honest share if the pot were paid out linearly over the term
            cols.append("%.3f (%.1fx)" % (fair, take / fair))
        print("  %4dh   | %.4f  | %5.1f%%                   | $%-6.0f | %5.1f%%           | %s" % (
            h, w, take * 100, take * P / WAD, take * 100 * 6 / h, " / ".join(cols)))
    # crossover: w(h) = h / T  -> JIT take equals time-pro-rata
    print("  Crossover (JIT take = time-pro-rata share, i.e. w(h) = h/T):")
    for T_d in (1, 7, 30):
        T = T_d * DAY
        h = 1
        while h < T and vesting_weight(h) * T >= h * RAY:
            h += 60
        print("   term %2dd: JIT beats time-pro-rata for every h < %s" % (T_d, ("%.1fh" % (h / HOUR)) if h < T else "T (never crosses inside the term: w(h) -> 1 while h/T < 1)"))
    # capital cost check: J needed to match H; H = opted-in cUSD. Take vs opportunity cost of J.
    print("  Cost side: JIT capital J = H (matching stake) at 5%/yr opportunity cost; net = take - cost:")
    for H_m in (1, 10, 50):
        J = H_m * 1_000_000
        for h in (6, 12, 24):
            take = 0.5 * vesting_weight(h * HOUR) / RAY * P / WAD
            c = J * r_alt * h / (24 * 365)
            print("   H=J=$%2dM h=%2dh: take $%-7.0f cost $%-6.0f net $%-7.0f (%.0f%% of pot)" % (H_m, h, take, c, take - c, (take) / (P / WAD) * 100))
    print("  Reading: the 12 h exponential vest cuts the 6 h take from 50%% (instant/short vest) to %.1f%% and the 12 h take" % (
        0.5 * vesting_weight(6 * HOUR) / RAY * 100))
    print("  to %.1f%%; but it is time-constant 12 h against a premium that pays for 30 days, so a JIT holder still" % (0.5 * vesting_weight(12 * HOUR) / RAY * 100))
    print("  earns 10-60x the time-pro-rata share and never falls below it inside a 7 or 30 day term. Only a vest")
    print("  time-constant comparable to the loan term (or streaming the fixed premium over the term) removes the JIT edge.")
    print("  Floating markets accrue premium continuously, so the JIT surplus there is bounded by one accrual step.")
