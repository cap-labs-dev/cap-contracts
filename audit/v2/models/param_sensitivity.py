...
def main():
    ...
    # ---------------- averagingPeriod ----------------
    row("averagingPeriod", "IRM.setAveragingPeriod (GOVERNOR @ 3dad5ef; was ADMIN-by-omission)", "[5 min, 1 day]",
        "entire band: EMA depression is profitable at every period for L >= $10M (ema_manipulation, exponential fold "
        "cuts the take to 63% at t=P but not the sign)", "YES (economic)")
    # rayPow precision at the extreme retention values (3dad5ef): 5 min -> 1e27 - 3.33e24 per second
    from capmath import ray_pow, retention_per_second, averaging_weight, VESTING_PERIOD
    from decimal import Decimal, getcontext
    getcontext().prec = 80
    worst = 0
    for P in (5 * 60, HOUR, DAY, VESTING_PERIOD):
        r = retention_per_second(P)
        for n in (1, 60, P, DAY, 7 * DAY, 30 * DAY, 365 * DAY):
            got = ray_pow(r, n)
            exact = int((Decimal(r) / Decimal(RAY)) ** n * RAY)
            worst = max(worst, abs(got - exact))
    r5 = retention_per_second(5 * 60)
    row("retentionPerSecond (derived)", "IRM._setAveragingPeriod / PremiumVesting._weight (rayPow)",
        "1e27 - 1e27/period: 5 min -> %d; 1 day -> %d" % (r5, retention_per_second(DAY)),
        "none: max |rayPow - exact| over periods {5m,1h,1d,12h} x elapsed {1s..1y} = %d ray-wei (%.1e rel); "
        "5-min retention^86400 = %d (exact 5.2e-126) so weight saturates to exactly 1e27 after a day; no overflow (base < 1e27)" % (
            worst, worst / 1e27, ray_pow(r5, DAY)), "no")
    ...
    # ---------------- vesting period ----------------
    row("VESTING_PERIOD", "PremiumVesting constant 12 h (setter REMOVED @ 3dad5ef)", "fixed: 43200 s",
        "round-1 'vestingPeriod = 1 s' row is gone. Residual: exponential 12 h still vests 39%% in 6 h, so a JIT "
        "depositor takes %.1f%% of a front-loaded fixed premium per 6 h of matched capital (run_dynamics part C)" % (
            (RAY - ray_pow(RAY - RAY // VESTING_PERIOD, 6 * HOUR)) / RAY * 50), "no (fixed), JIT residual")
