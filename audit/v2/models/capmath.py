...
DEFAULTS = dict(
    ...
    grace=1 * DAY,
    vestingPeriod=12 * HOUR,  # 3dad5ef: constant VESTING_PERIOD, setter removed
    KILL_RATIO=100,
...
def ray_pow(a, n):
    """WadRayMath.rayPow @ 3dad5ef: square-and-multiply, half-up rayMul per step, the final
    squaring skipped (it would be thrown away and is the one most likely to underflow)."""
    c = RAY
    while n > 0:
        if n & 1 == 1:
            c = rayMul(c, a)
...
def retention_per_second(period):
    """IRM._setAveragingPeriod: retentionPerSecond = 1e27 - 1e27 / period (integer division)."""
    return RAY - RAY // period


def averaging_weight_v1(elapsed, period):
    """Round-1 (3c45dca) linear weight: min(elapsed/period, 1). Kept for comparison only."""
    return RAY if elapsed >= period else elapsed * RAY // period


def averaging_weight(elapsed, period):
    """IRM._averagingWeight @ 3dad5ef: 1e27 - retentionPerSecond.rayPow(elapsed).
    Residual after one period is (1 - 1/P)^P ~ 1/e, so weight(P) ~ 0.632, not 1."""
...
def vesting_weight(elapsed):
    """PremiumVesting._weight @ 3dad5ef: same form, VESTING_PERIOD fixed at 12 h (no setter)."""
    return RAY - ray_pow(RAY - RAY // VESTING_PERIOD, elapsed)


def vested(remainder, elapsed):
    """PremiumVesting._vested: mulDiv(remainder, weight, RAY, Floor)."""
...
class UtilizationAverage:
    ...
    def __init__(self, t0, period, weight=None):
        self.weight = weight or averaging_weight  # averaging_weight_v1 for the round-1 linear fold
        self.credit = 0
        self.supply = 0
    ...
    def accrue(self, now, live_credit, live_supply):
        ...
        if elapsed > 0:
            w = self.weight(elapsed, self.period)
    ...
    def average_supplies(self, now):
        w = self.weight(now - self.lastUpdate, self.period)
