...
/// @notice H7 accounting consequence: Vault.deposit mints before it collects. With a
/// sender-side callback token, the depositor holds ERC-6909 for tokens the vault does not yet have
/// and can withdraw other depositors' tokens mid-call. The outer transferFrom then repays them, so
/// the end state reconciles; the window is the finding.
contract VaultCallbackTest is BaseTest {
    Vault internal vault;
    CallbackToken internal token;
    ...
    function test_mintBeforeTransfer_windowLetsDepositorWithdrawOthersTokens() public {
