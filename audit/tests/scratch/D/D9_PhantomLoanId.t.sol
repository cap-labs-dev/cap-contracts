...
/// @notice WS-D. FixedMarket never checks `id < loanCount`. `extend` / `extendAdmin` on an unused id
/// (expiry 0, so "expired") create an expiry for it; `borrowMore` then draws on it; and the next
/// `borrow` reuses that same id (`loanCount++`) and OVERWRITES `expiry[id]`, re-terming the whole
/// debt already sitting on it. The owner/borrower pair gets a maximum-term loan for the price of a
/// minimum-term premium.
contract D9_PhantomLoanId is CapDeployer {
    FixedMarket market;
    ...
    function test_reTermViaPhantomId_paysMinimumTermForMaximumTerm() public {
        ...
        // honest reference: what a 30-day loan of 4_000 costs
        ...
        // 1. owner gives the unused id a 1-day expiry (extend treats expiry 0 as "expired" and rolls)
        ...
        // 3. borrower opens the "next" loan with a dust principal at the maximum term: same id,
        //    expiry overwritten to now + 30 days, the 4_000 rides along
        vm.prank(defaultBorrower);
        ...
        // borrower may hold this for 30 days without any further charge
