...
import {
    ...
    ERC7540AsyncRedeem,
    IERC4626
} from "../../../../../contracts/ERC7540/ERC7540AsyncRedeem.sol";
...
/// @title StablecoinV2 (scratch copy of contracts/cap/Stablecoin.sol @ a843c1d + migrate())
/// @dev Verbatim copy except for the added {migrate}; `layout at` contracts cannot be inherited.
/// @author kexley, Cap Labs
/// @notice Credit-backed ERC-7540 stablecoin
contract StablecoinV2 layout at erc7201("cap.storage.Stablecoin")
    is
    IStablecoin,
    AccessManagedUpgradeable,
    PremiumVesting,
    UUPSUpgradeable
{
    ...
    /// @dev SCRATCH: second-version initializer for a proxy that already ran a v1 `initializer`.
    /// Rewrites ERC20 name/symbol with the values already in storage ("cap USD"/"cUSD"), sets
    /// ERC4626 asset+decimals for the first time, and fills every slot {initialize} would have.
    function migrate(address _authority, address _asset, address _irm, address _reserveVault)
        external
        reinitializer(2)
    {
        __AccessManaged_init(_authority);
        __PremiumVesting_init(IERC20Metadata(_asset), name(), symbol(), address(this));
