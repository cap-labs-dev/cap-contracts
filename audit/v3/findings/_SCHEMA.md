...
**Invariant broken:** the invariant ID from audit/v3/00-plan.md, if applicable

## Also report
At the end of your file, a `## Invariants` section listing any invariant from audit/v3/00-plan.md
...
- You MAY write scratch Foundry tests under `audit/v3/tests/scratch/<WS>/` to check a hypothesis.
  `test/shared/CapDeployer.sol` gives you a full protocol deployment - use it.
- Run tests with: `FOUNDRY_TEST=audit/v3/tests forge test --match-path 'audit/v3/tests/scratch/<WS>/*' -vv` (the env var is REQUIRED - foundry.toml points test= at test/, do not edit foundry.toml)
