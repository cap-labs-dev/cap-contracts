...
/// @dev Mint-path regression tests for `CapToken` (cUSD).
/// These focus on input validation and simple price/cap scenarios.
contract VaultMintTest is VaultFixture {
    address user;
    ...
    function _fundAndApproveUsdt(address owner, uint256 amountIn) internal {
        usdt.mint(owner, amountIn);
        usdt.approve(address(cUSD), amountIn);
    }

    function _mintCusdFromUsdt(uint256 amountIn, uint256 minAmountOut, address recipient) internal {
        cUSD.mint(address(usdt), amountIn, minAmountOut, recipient, _deadline());
    }
    ...
    function test_vault_mint_on_empty_vault() public {
        ...
        _fundAndApproveUsdt(user, amountIn);
        _mintCusdFromUsdt(amountIn, minAmountOut, user);
    ...
    function test_vault_mint_with_different_prices() public {
        // If the input asset price increases, the same `amountIn` should mint fewer cUSD.
    ...
    function test_mint_with_invalid_asset() public {
        ...
        vm.expectRevert();
        cUSD.mint(address(invalidAsset), amountIn, minAmountOut, user, _deadline());
    }
