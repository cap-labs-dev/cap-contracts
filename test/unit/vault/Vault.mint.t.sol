...
contract VaultMintTest is TestDeployer {
    ...
    function test_mint_with_recipient_zero_address() public {
        vm.startPrank(user);
        ...
        vm.expectRevert();
        cUSD.mint(address(usdt), 100e6, 0, address(0), block.timestamp + 1 hours);
    }

    function test_mint_with_asset_zero_address() public {
        vm.startPrank(user);
        ...
        vm.expectRevert();
        cUSD.mint(address(0), 100e6, 0, user, block.timestamp + 1 hours);
    }
}
