...
contract ConfigureAccessControl {
    ...
    function _initInfraAccessControl(InfraConfig memory infra, UsersConfig memory users) internal {
        ...
        // the deployer only needed ADMIN to wire the registry; do not leave it there
        if (users.deployer != users.admin) {
            manager.revokeRole(CapRoles.ADMIN, users.deployer);
        }
    }
}
