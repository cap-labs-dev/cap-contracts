 ...
 */
contract DeployL2Token is Script, LzUtils {
    function run() public {
        ...
        L2Token cToken = new L2Token(name, symbol, config.endpointV2, msg.sender);
        console.log(string.concat("Bridged ", name, " ", symbol), address(cToken));

        name = string.concat("Staked ", name);
        symbol = string.concat("s", symbol);
        L2Token scToken = new L2Token(name, symbol, config.endpointV2, msg.sender);
